...
export class DiscoveredLeadsService {
  ...
  async findAll(userId: string, query: QueryDiscoveredDto) {
    ...
    const [items, total] = await Promise.all([
      this.prisma.discoveredLead.findMany({
        where,
        skip,
        take: limit,
  ...
  async importToCrm(userId: string, id: string) {
    const discovered = await this.prisma.discoveredLead.findFirst({
      where: { id, userId },
    });
    ...
    const user = await this.prisma.user.findUnique({
      where: { id: userId },
      select: { id: true, ownerId: true, activeWorkspaceId: true },
    });
    if (!user) throw new NotFoundException('User not found');

    const lead = await this.prisma.lead.create({
      data: {
        workspaceId: user.activeWorkspaceId ?? user.ownerId ?? user.id,
        ...
        notes:
          [
            ...
            discovered.linkedinUrl
              ? `LinkedIn: ${discovered.linkedinUrl}`
              : null,
            discovered.location ? `Location: ${discovered.location}` : null,
            discovered.contactTitle
              ? `Title: ${discovered.contactTitle}`
              : null,
          ]
            .filter(Boolean)
            .join('\n') || null,
  ...
  async bulkImport(userId: string, ids: string[]) {
    const results = await Promise.allSettled(
      ids.map((id) => this.importToCrm(userId, id)),
    );
  ...
  async exportCsv(userId: string, query: QueryDiscoveredDto): Promise<string> {
    ...
    const header =
      'Business Name,Contact Name,Title,Email,Phone,Website,Location,Company Size,Source,Intent Score,LinkedIn\n';
